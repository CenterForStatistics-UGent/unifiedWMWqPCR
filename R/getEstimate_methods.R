...
#' @param esttype a character string indicating the measure you want to extract. It can take the values \code{logor} for the log odds ratio, \code{or} for the odds ratios, \code{logodds} for the log odds, \code{odds} for the odds or \code{p} for the percentages. 
#' @param se.fit logical value indicating whether the standard errors of the logor or the log odds should be returned as well. Ignored when type has a value different from \code{logor} or \code{logodds}. Note that you can also use the accessor \code{\link{se}} to get only the standard errors.
#' @param ci numerical value indicating the confidence interval (0.95 is 95\% confidence interval). If set to TRUE, the 95% confidence interval is returned. If set to NULL, no confidence interval is returned.
#' @param drop a logical value. If set to \code{TRUE} and neither \code{se} nor \code{ci} is calculated, the function returns a vector instead of an uwmwEstimate object.
#' @param ... passes on arguments to the next method
#' 
#' @details The function can only calculate standard errors for the log OR and the log odds. In all other cases, \code{se.fit} is ignored. The function takes into account a possible ordering in the object (see also \code{\link{sort.uwmwRes}}). So take into account that you get the estimates in the specified order. In case you want this different, either use the function \code{\link{unorder}} on the object first, or check if any of the \code{\link{uwmw_Accessors}} can help you out.
#' 
#' The argument \code{se.fit} is mainly to be used to save calculation time. Normally there's no need to set it to \code{FALSE}.
#'
#' 
#' @return In general, a \code{\link{uwmwEstimate}} object with the requested estimate. See \code{\link{uwmwEstimate}} for details. In case \code{drop=TRUE} and neither the standard error nor the confidence interval is calculated, a numeric named vector.
#' 
#' @examples
#' data(NBmat)
